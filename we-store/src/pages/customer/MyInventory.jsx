...
export default function MyInventory() {
  ...
  const filtered = useMemo(() => {
    return myItems.filter((item) => {
      ...
      const matchesUnit = unitFilter === 'All' || item.unitId === Number(unitFilter)
      return matchesSearch && matchesCategory && matchesUnit
    })
  }, [myItems, search, categoryFilter, unitFilter])

  function openAddModal() {
    if (myUnits.length === 0) {
      showToast('Reserve a unit before adding inventory.', 'error')
      return
    }
    setForm({ ...emptyForm, unitId: myUnits[0].id })
    setEditingId(null)
    setModalOpen(true)
  }
    ...
    <div>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-slate-500">Track everything you have stored across your units.</p>
        </div>
        <button
      ...
      <ConfirmDialog
        open={!!deleteTarget}
        title="Delete item"
        message={`Remove "${deleteTarget?.name}" from your inventory? This can't be undone.`}
        confirmLabel="Delete"
